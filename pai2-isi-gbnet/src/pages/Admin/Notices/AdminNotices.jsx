...
export default function AdminNotices() {
  ...
  useEffect(() => {
    async function fetchUsers() {
      ...
      const res = await axios.get(
        "http://localhost:8080/upc/v1/worker-role/user/all"
      );
  ...
  async function handleSubmit() {
    if (description?.length < 20) {
      ...
      const response = await axios.post(
        "http://localhost:8080/upc/v1/worker-role/save-notice",
        data
      );
  ...
  function handleError(type) {
    ...
    const mes =
      type === 1
        ? "Tresć zgłoszenia musi wynosić conajmniej 20 znaków!"
    ...
    <div className="flex flex-col xl:flex-row w-full bg-gray-300 h-full items-center justify-center xl:py-32 xl:p-4 xl:space-x-8 space-y-8 2xl:space-y-0">
      <div className="flex flex-col w-3/4 2xl:w-1/2 h-full">
        ...
        <div className="flex flex-col justify-center items-center mt-3">
          <Button onClick={handleSubmit} color="amber">
            Wyślij
          </Button>
        </div>
      </div>
